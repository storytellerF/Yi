...
class GradientDrawable : Drawable() {
    ...
    override fun inflate(element: Element) {
        ...
        if (shape == GradientDrawable.RING) {
            ...
            if (innerRadius == DEFAULT_INT_VALUE) {
                innerRadiusRatio =
                    Utils.parseAttributeAsFloat(element.getAttribute(INNER_RADIUS_RATIO), innerRadiusRatio)
            }
        ...
        BufferedImage(resolvedWidth, resolvedHeight, BufferedImage.TYPE_INT_ARGB).also { resizedImage ->
            resizedImage.createGraphics().also { resizedGraphics ->
                ...
                outputImage.createGraphics()?.also { graphics ->
                    graphics.drawImage(
                        resizedImage,
                        (outputImage.width - resolvedWidth) / 2,
                        (outputImage.height - resolvedHeight) / 2,
                        null
                    )
                    graphics.dispose()
                }
    ...
    private fun drawShape(graphics: Graphics2D) {
        ...
        if (gradientColors.isEmpty()) {
            ...
            graphics.paint = getGradientPaint(
                arrayOf(
                    startGradientColor ?: Color.BLACK,
    ...
    private fun createStroke(): Stroke {
        ...
        return BasicStroke(
            resolvedStrokeWidth, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 1F,
    ...
    private fun getOval(forStroke: Boolean = false): Shape {
        return if (forStroke) {
            ...
            Ellipse2D.Float(
                halfStroke,
                halfStroke,
                resolvedWidth.toFloat() - resolvedStrokeWidth,
                resolvedHeight.toFloat() - resolvedStrokeWidth
            )
        } else {
